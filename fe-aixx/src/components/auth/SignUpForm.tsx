...
import { persistStudentSession, registerStudent } from "@/services/studentService";
...
const passwordStrength = (pwd: string) => {
  ...
  return minLength <= pwd.length && hasUpper && hasLower && hasNumber && hasSpecial;
};
...
export default function SignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!passwordStrength(password)) {
      newErrors.password = "Password must be at least 8 characters, include upper, lower, number and special character";
    }
  ...
  return (
    <div className="flex flex-col flex-1 w-full overflow-y-auto no-scrollbar px-4 sm:px-6 lg:px-8 py-10 h-full">
      <div className="w-full max-w-md sm:pt-4 mx-auto mb-5">
        <Link href="/" className="inline-flex items-center text-sm text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400">
          <ChevronLeftIcon />
          Back to Home
        </Link>
      </div>

      <div className="flex flex-col flex-1 w-full max-w-md mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8 sm:p-10 mb-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              <Input placeholder="Confirm your password" type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
          ...
          <div>
            <button type="submit" className="flex items-center justify-center w-full px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600 disabled:opacity-50" disabled={loading}>
